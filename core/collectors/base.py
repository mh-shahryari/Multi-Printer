...
def _counters_event(ip: str, total: int, prev: dict, alerts: list, curr_codes: list,
    ...
    if total < prev_total:
        ...
        is_reboot = (uptime is not None and prev_uptime is not None and 
                     uptime < prev_uptime - 60*100)
        reason = f"{'ریبوت' if is_reboot else 'کاهش'} شمارنده: قبلی {prev_total:,} -> جدید {total:,}"
        add_event(ip, "COUNTER_RESET", {
            "message": f"شمارنده از {prev_total:,} به {total:,} کاهش یافت" + 
                       (f" (ریبوت دستگاه)" if is_reboot else ""),
            "severity": "error",
    ...
    # ✅ باگ #11: ذخیره a3_total و a4_total برای تشخیص سایز کاغذ
    new_prev = {
        "print_total": total,
    ...
    # فقط اگر مقدار داشته باشیم اضافه کن (برای جلوگیری از پاک شدن مقادیر قبلی)
